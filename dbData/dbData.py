...
def parse_qyc_page(link):
    ...
    data['WXHWYSCLLX'] = trs[36].findAll('td')[1].text
    data['XJXS'] = trs[36].findAll('td')[3].text

    data['PQGCKWZ'] = trs[36].findAll('td')[1].text

    data['TDYSJZ'] = trs[37].findAll('td')[1].text

    return data


# 解析挂车页面
def parse_gc_page(link):
    data = get_data()
    data['CLLB'] = '挂车'
    response = requests.get(link).content.decode('utf-8')
    soup = BeautifulSoup(response, 'html.parser')
...
if __name__ == '__main__':
    ...
    for pageNo in range(0, totalPage):
        ...
        # 定义运输车的产品名车列表
        transport_name = ['自卸汽车', '纯电动自卸汽车', '载货汽车', '厢式运输车', '仓栅式运输车', '随车起重运输车',
                          '平板运输车', '冷藏车', '翼开启厢式车', '畜禽运输车', '疫苗冷链车', '气瓶运输车', '散装饲料运输车', '毒性和感染性物品厢式运输车',
                          '易燃气体厢式运输车', '爆破器材运输沙漠车', '沙漠车', '自卸车', '混凝土搅拌运输车', '纯电动自卸车', '车厢可卸式汽车', '普通液体运输车',
                          '自卸式垃圾车', '邮政车', '易燃液体厢式运输车', '纯电动混凝土搅拌运输车', '医疗废物转运车', '换电式纯电动自卸汽车', '低温液体运输车',
                          '越野自卸汽车', '铝合金运油车', '供液车', '加油车', '腐蚀性物品罐式运输车', '运油车', '清障车', '杂项危险物品厢式运输车', '腐蚀性物品厢式运输车',
                          '渣料运输车', '插电式混合动力冷藏车', '插电式混合动力自卸式垃圾车', '教练车', '畜禽式运输车', '低密度粉粒物料运输车', '干混砂浆运输车',
                          '纯电动冷藏车', '燃料电池自卸汽车', '燃料电池厢式运输车', '飞机加油车', '雏禽运输车', '鲜奶运输车', '爆破器材运输车', '易燃液体罐式运输车',
                          '腐蚀性物品运输车', '易燃固体厢式运输车', '杂项危险物品罐式运输车', '氧化性物品罐式运输车', '车厢可卸式垃圾车', '污泥自卸车', '压裂砂罐车',
                          '爆破器材厢式运输车', '鲜活水产品运输车', '污泥运输车', '食用油运输车', '越野仓栅式运输车', '医疗废弃物转运车', '毒性气体厢式运输车', '现场混装重铵油炸药车',
                          '现场混装铵油炸药车', '背罐车', '放射性物品厢式运输车', '现场混装乳化炸药车', '燃料电池保温车', '燃料电池冷藏车', '甲醇自卸汽车',
                          '换电式纯电动混凝土搅拌运输车', '纯电动平板运输车', '铝合金易燃液体罐式运输车', '吸污车', '瓶装饮料运输车', '换电式纯电动自卸式垃圾车', '液化气体运输车',
                          '下灰车', '散装水泥运输车', '洒水车', '纯电动自卸式垃圾车', '平头天然气自卸汽车', '平头柴油自卸汽车', '普通液体罐式运输车', '平头柴油载货汽车',
                          '平头平板运输车',
                          '杂项危险品厢式运输车', '纯电动翼开启厢式车', '插电式混合动力自卸汽车']
        ...
        for page in page_list:
            ...
            print(file_path)
            if (product_name == '商务车' or '客车' in product_name):
                result.append(parse_kc_page(file_path))
            elif (product_name == '轻型越野汽车' or '乘用车' in product_name):
                result.append(parse_cyc_page(file_path))
            elif (product_name in transport_name):
                result.append(parse_ysc_page(file_path))
            elif ('牵引车' in product_name):
                result.append(parse_qyc_page(file_path))
            elif ('挂车' in product_name):
                result.append(parse_gc_page(file_path))
        print(result)
